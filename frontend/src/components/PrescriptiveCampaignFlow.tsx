...
interface ZipResult {
  ...
  area: string;
  insight: string;
  topPlatform: { id: string; reason: string };
}
...
export default function PrescriptiveCampaignFlow() {
  const [view, setView] = useState<'dashboard' | 'campaign'>('dashboard');
  const [selectedZip, setSelectedZip] = useState<ZipResult | null>(null);
  ...
  const zipResults: ZipResult[] = [
    { 
      ...
      cohortColor: '#64748b',
      area: 'Manhasset',
      insight: 'Affluent Long Island suburb where families balance luxury tastes with practical spending. Median age 42, household income $145K. These patients research extensively before booking and respond well to transparent pricing and package deals.',
    ...
    { 
      ...
      cohortColor: '#7c3aed',
      area: 'New Rochelle', 
      insight: 'Sophisticated Westchester community 8.8 miles from your practice. Median age 38, household income $185K. High social media engagement. These discerning clients prioritize premium results and expect white-glove service.',
    ...
    { 
      ...
      cohortColor: '#059669',
      area: 'Merrick',
      insight: 'Family-oriented Nassau County neighborhood. Median age 45, household income $125K. Residents are cost-conscious but beauty-focused, seeking treatments that fit their budget. High word-of-mouth referral potential.',
      topPlatform: { id: 'facebook', reason: 'Maximum reach at lowest cost' }
    },
  ];

  const platforms: Platform[] = [
    { id: 'facebook', name: 'Facebook', color: '#1877F2', icon: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z', overview: 'Leverage Facebook\'s advanced targeting to find new clients similar to your top customers.', kpis: ['Cost per Lead: $12-18', 'Engagement Rate: 3-5%', 'Booking Rate: 8-12%'], nextSteps: ['Test carousel vs single image ads', 'Create lookalike audience from top clients', 'A/B test ad copy variations'], insights: ['Max CPA $90 (ROAS 5× on $430); monthly ad cap $540', '0.0 competitors/10k (Low competition) — prioritize expansion', 'Revenue potential $2,579/mo (P50: 6 × $430)', '0.0 mi (~0 min) — baseline no-show risk; use standard reminders'] },
    { id: 'instagram', name: 'Instagram', color: '#E1306C', icon: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z', overview: 'Instagram\'s visual platform is perfect for showcasing before/after results and procedure videos.', kpis: ['Video Completion: 45-60%', 'Story Reply Rate: 2-4%', 'Profile Visits: 200-400/week'], nextSteps: ['Create Reels showing transformations', 'Use Story polls for engagement', 'Run Story ads with booking link'], insights: ['Max CPA $70 (ROAS 5× on $360); monthly ad cap $280', '0.0 competitors/10k (Low competition) — prioritize expansion', 'Revenue potential $1,441/mo (P50: 4 × $360)', '8.8 mi (~29 min) — elevated no-show risk; double reminders; consider deposit'] },
    { id: 'tiktok', name: 'TikTok', color: '#000000', icon: 'M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z', overview: 'TikTok\'s younger audience is actively searching for aesthetic treatments.', kpis: ['Video Views: 10k-50k', 'Engagement Rate: 5-8%', 'Click-Through Rate: 1-2%'], nextSteps: ['Post testimonial videos', 'Use trending audio', 'Partner with local influencers'], insights: ['Max CPA $45 (ROAS 5× on $230); monthly ad cap $320', 'Younger demographic (18-34) — focus on preventative treatments', 'Revenue potential $1,840/mo (P50: 8 × $230)', 'Video-first platform — invest in quality production'] },
    { id: 'google', name: 'Google Ads', color: '#4285F4', icon: 'M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z', overview: 'Capture high-intent searchers actively looking for aesthetic treatments in your area.', kpis: ['Cost per Conversion: $25-45', 'Conversion Rate: 8-15%', 'Call Rate: 12-18%'], nextSteps: ['Optimize landing pages', 'Add call extensions', 'Track phone conversions'], insights: ['Max CPA $65 (ROAS 5× on $330); monthly ad cap $450', 'High intent searches — capitalize on immediate booking interest', 'Revenue potential $2,310/mo (P50: 7 × $330)', 'Strong phone call volume — ensure staff handles inquiries promptly'] },
  ];

  const adVariations: Record<string, AdVariation[]> = {
    facebook: [
  ...
  const handleStartCampaign = (zip: ZipResult, platformId: string) => {
    setSelectedZip(zip);
  ...
  if (view === 'campaign' && currentPlatform) {
    return (
            ...
            <div style={{backgroundColor: currentPlatform.color}} className="px-8 py-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-sm font-bold text-white uppercase tracking-wide mb-1">{currentPlatform.name} Campaign</div>
                    <h2 className="text-lg text-white/90">ZIP {selectedZip.zip} • {selectedZip.cohort}</h2>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    {platforms.map(p => (
                      ...
                      >
                        {p.name}
                        {p.id === selectedZip?.topPlatform.id && (
              ...
              <div className="flex gap-8">
                <button
                  ...
                  className={`py-4 text-sm font-medium transition-colors relative ${
                    activeTab === 'campaign' ? 'text-gray-900' : 'text-gray-500'
                  }`}
                >
                  Campaign
                  {activeTab === 'campaign' && (
                ...
                <button
                  ...
                  className={`py-4 text-sm font-medium transition-colors relative ${
                    activeTab === 'creative' ? 'text-gray-900' : 'text-gray-500'
                  }`}
                >
                  Creative
                  {activeTab === 'creative' && (
            ...
            <div className="p-8 space-y-6">
              ...
              {activeTab === 'campaign' && (
                <>
                  ...
                  >
                    <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Strategy Overview</h3>
                    <p className="text-sm text-gray-700 leading-relaxed">{currentPlatform.overview}</p>
                    <div className="grid md:grid-cols-3 gap-3 mt-4">
                      {currentPlatform.kpis.map((kpi, idx) => (
                        <div key={idx} className="text-xs">
                          <span className="font-semibold text-gray-900">{kpi.split(':')[0]}:</span>
                          <span className="text-gray-600"> {kpi.split(':')[1]}</span>
                        </div>
                      ))}
                    </div>
                    
                    {currentPlatform.insights.length > 0 && (
                      <div className="mt-5 pt-5 border-t" style={{ borderColor: `${currentPlatform.color}20` }}>
                        <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Key Insights</h4>
                  ...
                  <div>
                    <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-4">Ad Copy Variations</h3>
                    <div className="space-y-3">
                      {(adVariations[activePlatform] || adVariations.facebook).map((ad, idx) => (
                        <div key={idx} className="p-4 border border-gray-200 rounded-lg hover:border-gray-300 transition-colors">
                          <div className="flex justify-between items-start gap-4">
                            <div>
                              <h4 className="font-semibold text-gray-900 text-sm mb-1">{ad.headline}</h4>
                              <p className="text-sm text-gray-600">{ad.body}</p>
                            </div>
                            <button
                              onClick={() => copyToClipboard(`${ad.headline}\n\n${ad.body}`, `ad-${idx}`)}
                              className="px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white text-xs font-medium rounded transition-colors whitespace-nowrap flex-shrink-0"
                            >
              ...
              {activeTab === 'creative' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="text-sm text-gray-700">• Before/after results</div>
                        <div className="text-sm text-gray-700">• Staff consultation photos</div>
                        <div className="text-sm text-gray-700">• Pricing transparency graphics</div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="text-sm text-gray-700">• Client testimonials</div>
                        <div className="text-sm text-gray-700">• Treatment walkthroughs</div>
                        <div className="text-sm text-gray-700">• Office tour videos</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                    <div>
                      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Photography Brief</div>
                      <p className="text-sm text-gray-700">Bright, authentic imagery showcasing real results and approachable staff.</p>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Video Concept</div>
                      <p className="text-sm text-gray-700">Educational content emphasizing transparency and client success stories.</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              >
                ...
                <div className="mb-5 flex-grow">
                  ...
                  <div className="flex items-center gap-2 text-xs text-blue-600 bg-blue-50 px-3 py-2 rounded-lg">
                    ...
                    <span className="font-medium">Best channel: {result.topPlatform.id.charAt(0).toUpperCase() + result.topPlatform.id.slice(1)}</span>
                  </div>
                </div>
